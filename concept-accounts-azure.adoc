...
从BlueXP部署Connector时、您需要使用有权部署Connector虚拟机的Azure帐户或服务主体。中列出了所需的权限 link:task-install-connector-azure-bluexp.html#step-2-create-a-custom-role["适用于 Azure 的连接器部署策略"]。
