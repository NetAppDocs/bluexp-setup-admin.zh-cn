...
\https://core.usgovcloudapi.net | 管理Azure政府区域中的资源。 
...
没有传入到连接器的流量、除非您启动该流量、或者该连接器用作AutoSupport 消息的代理。HTTP 和 HTTPS 可用于访问 https://docs.netapp.com/us-en/bluexp-setup-admin/concept-connectors.html#the-local-user-interface["本地 UI"]，在极少数情况下使用。只有在需要连接到主机进行故障排除时，才需要使用 SSH 。
