...
+
https://["了解Cloud Volumes ONTAP 增强功能"^]
...
+
** https://["将连接器安装在无法访问Internet的位置"^]
** https://["访问Connector上的BlueXP控制台"^]
** https://["添加未分配的许可证"^]
** https://["开始使用Cloud Volumes ONTAP"^]


* 现在、您可以通过Connector从BlueXP添加和管理Amazon S3存储分段。
+
https://["了解如何从BlueXP在AWS帐户中添加新的Amazon S3存储分段"^]。
...
+
https://["查看Connector的Google Cloud权限"]。
...
现在、当与您的帐户关联的刷新令牌在3个月后过期时、Cloud Manager将提示您更新与您的NetApp 支持站点 帐户关联的凭据。 https://["了解如何管理 NSS 帐户"^]
...
+
https://["了解如何利用"我的商机"发现适用于ONTAP 的FSx"^]
...
https://["以下示例显示了用于创建Connector的AWS IAM角色权限"]。

我们还创建了一个页面、用于提供指向每个策略的链接。 https://["查看Cloud Manager的权限摘要"]。
...
* 我们已在具有支持的操作系统的VM实例上对Google Cloud中的Connector进行了认证 https://["屏蔽VM功能"^]
* 此版本的连接器还包括Cloud Volumes ONTAP 增强功能。 https://["了解这些增强功能"^]
...
此版本的连接器包含Cloud Volumes ONTAP 增强功能。 https://["了解这些增强功能"^]
...
* https://["了解有关 NetApp 使用 Microsoft Azure AD 进行身份管理的更多信息"^]
...
https://["了解有关使用 Cloud Volumes ONTAP 验证 AutoSupport 的更多信息"^]
