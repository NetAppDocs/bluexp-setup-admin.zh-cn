...
| —cloudkms.encryptoKeys.get—cloudkms.encryptoKeys.getIamPolicy—cloudkms.encryptoKeys.list—cloudkms.cryptoKeys.setIamPolicy—cloudkms.keyrings.get—cloudkms.keyrings.getIamPolicy—cloudkms.keyrings.list—cloudkms.keyRings.setIamPolicy | 在BlueXP备份和恢复激活向导中选择您自己的客户管理密钥、而不是使用默认的Google管理加密密钥。 
|===
