...
summary: 您应了解在受限模式下开始使用BlueXP所需的资源、以便为您的环境做好准备。 
---
= 准备在受限模式下部署
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
您应了解在受限模式下开始使用BlueXP所需的资源、以便为您的环境做好准备。
...
通过HTTP (80)和HTTPS (443)可以访问BlueXP控制台。只有在需要连接到主机进行故障排除时，才需要使用 SSH （ 22 ）。同时、如果在出站Internet连接不可用的子网中部署Cloud Volumes ONTAP 系统、则需要通过端口3128进行入站连接。
