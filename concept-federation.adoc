...
如果您的身份提供程序未显示在上述列表中、 https://["请按照以下通用说明进行操作"^]
...
| 2. | BlueXP管理员  a| 
转至 https://["NetApp联合设置页面"^] 并创建与BlueXP的连接。
...
| 4. | BlueXP管理员 | 从测试并启用连接 https://["NetApp联合设置页面"^]请注意、此页面会在测试连接和启用连接之间刷新。 
|===
...
在BlueXP管理员启用连接后、管理员可以随时从更新此连接 https://["NetApp联合设置页面"^]
