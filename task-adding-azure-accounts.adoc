...
通过BlueXP、您可以选择要在其中部署Cloud Volumes ONTAP 的Azure凭据和Azure订阅。除非关联，否则您无法为托管身份配置文件选择其他 Azure 订阅 https://["托管身份"^] 这些订阅。
...
.开始之前
要创建 Active Directory 应用程序并将此应用程序分配给角色，您必须在 Azure 中拥有适当的权限。有关详细信息，请参见 https://["Microsoft Azure 文档：所需权限"^]。
...
.结果
现在，您可以从 " 详细信息和凭据 " 页面切换到不同的凭据集 https://["创建新的工作环境时"^]
