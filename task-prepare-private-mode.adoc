...
NOTE: 如果要在中使用BlueXP https://["AWS Commercial Cloud Services (C2S)环境"^] 然后、您应按照单独的说明进行操作、以说明如何准备、安装连接器和启动Cloud Volumes ONTAP。 https://["了解如何在AWS C2S环境中开始使用Cloud Volumes ONTAP"^]
...
----
+
https://["Google Cloud文档：创建和管理自定义角色"^]
...
.步骤
. https://["在项目中启用以下 Google Cloud API"^]
+
