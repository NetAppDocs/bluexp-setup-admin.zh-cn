...
[role="lead"]
BlueXP帐户管理员需要先部署_Connector_、然后才能使用大多数BlueXP功能。借助此连接器、BlueXP可以管理公有 云环境中的资源和流程。 link:concept-connectors.html["了解何时需要连接器"]。

此页面介绍如何直接从BlueXP在Azure中创建Connector。 link:concept-connectors.html#how-to-create-a-connector["了解部署 Connector 的其他方法"]。
...
要部署Connector、您需要为BlueXP提供一个登录名、该登录名必须具有在Azure中创建Connector VM所需的权限。

您有两种选择：

. 出现提示时，使用 Microsoft 帐户登录。此帐户必须具有特定的 Azure 权限。这是默认选项。
+
 a Connector using your Azure account,请按照以下步骤开始操作。

. 提供有关 Azure AD 服务主体的详细信息。此服务主体还需要特定权限。
+
 a Connector using a service principal,请按照以下步骤开始操作。
...
设置您的网络，以便 Connector 可以管理公有云环境中的资源和流程。除了为Connector提供vNet和子网之外、您还需要确保满足以下要求。
...
例如、如果您在公司网络中安装了连接器、则必须设置与启动Cloud Volumes ONTAP 的vNet的VPN连接。
...
| https://*.api.bluexp.netapp.com \https://api.bluexp.netapp.com \https://*.cloudmanager.cloud.netapp.com \https://cloudmanager.cloud.netapp.com | 在BlueXP中提供SaaS功能和服务。注意：Connector当前正在与cloudmanager.cloud.netapp.com"联系、但在即将发布的版本中、它将开始与api.bluexp.netapp.com"联系。 
...
在从BlueXP部署Connector之前、您需要确保Azure帐户具有正确的权限。
...
通过BlueXP、您可以直接从其用户界面在Azure中创建Connector。
...
+
.. 单击*继续*以使用产品指南为部署做准备。每个步骤都包含文档此页面中的信息。
...
您需要将连接器与工作空间关联，以便 Workspace 管理员可以使用这些连接器创建 Cloud Volumes ONTAP 系统。如果您只有帐户管理员，则不需要将 Connector 与工作空间相关联。默认情况下、帐户管理员可以访问BlueXP中的所有工作空间。 link:task-setting-up-netapp-accounts.html#associating-connectors-with-workspaces["了解更多信息。"]。
...
您还可以选择为BlueXP提供具有所需权限的Azure服务主体的凭据、而不是使用Azure帐户登录。



=== 使用服务主体授予 Azure 权限

通过在Azure Active Directory中创建和设置服务主体并获取BlueXP所需的Azure凭据、授予在Azure中部署Connector所需的权限。
...
创建一个Azure Active Directory (AD)应用程序和服务主体、BlueXP可使用它来部署Connector。
...
从BlueXP创建Connector时、您需要提供应用程序(客户端) ID和目录(租户) ID。BlueXP使用ID以编程方式登录。
...
* Azure 订阅。
* 您选择的 Azure 区域中的 vNet 和子网。
* 有关HTTP代理的详细信息、如果您的组织需要一个代理来处理所有传出Internet流量：
+
** IP 地址
** 凭据
** HTTPS证书
...
. 在*部署连接器*页面上：
+
.. 在*身份验证*下、单击* Active Directory服务主体*并输入有关授予所需权限的Azure Active Directory服务主体的信息：
+
