...
+
https://["适用于商业区域的Azure Marketplace页面"^]
...
+
link:reference-host-requirements-azure["查看虚拟机要求"]。
...
+
此设置非常重要，因为托管身份允许 Connector 虚拟机在不提供任何凭据的情况下向 Azure Active Directory 标识自己。完成后、您需要将创建的自定义角色分配给 https://["详细了解 Azure 资源的托管身份"^]。
...
+
https://["Microsoft Azure文档：使用Azure门户为虚拟机上的Azure资源配置托管身份"^]
...
. 从下载Connector软件 https://["NetApp 支持站点"^]，然后将其复制到 Linux 主机。
+
