...
[role="lead"]
在大多数情况下、Cloud Manager帐户管理员需要在云或内部网络中部署_Connector_。连接器是 Cloud Manager 日常使用的关键组件。借助此连接器， Cloud Manager 可以管理公有云环境中的资源和流程。
...
Cloud Manager帐户管理员可以通过多种方式创建Connector：
