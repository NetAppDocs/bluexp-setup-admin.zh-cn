...
+
虽然创建工作环境不需要使用Connector、但创建和管理卷、复制数据以及将适用于ONTAP 的FSx与诸如Data sense和Cloud Sync 等BlueXP服务集成也需要使用Connector。
...
直接从BlueXP创建Connector需要特定权限、而Connector实例本身也需要另一组权限。如果您直接从BlueXP在AWS或Azure中创建Connector、则BlueXP将使用所需权限创建Connector。您无需执行任何其他操作。
...
* 您拥有一个多云环境(AWS和Azure)、并且更喜欢在AWS中使用一个Connector、在Azure中使用另一个Connector。每个都管理在这些环境中运行的 Cloud Volumes ONTAP 系统。
