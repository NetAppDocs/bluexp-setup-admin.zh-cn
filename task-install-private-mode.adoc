...
summary: 要在专用模式下设置BlueXP、您需要安装Connector、通过访问Connector上运行的用户界面来设置BlueXP、然后提供云权限。 
---
= 在专用模式下部署连接器
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
要在专用模式下设置BlueXP、您需要安装Connector、通过访问Connector上运行的用户界面来设置BlueXP、然后提供云权限。
...
如果要在中使用BlueXP https://aws.amazon.com/federal/us-intelligence-community/["AWS Commercial Cloud Services (C2S)环境"^] 然后、您应按照单独的说明开始在该环境中使用。 https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-getting-started-aws-c2s.html["了解如何在AWS C2S环境中开始使用Cloud Volumes ONTAP"^]
