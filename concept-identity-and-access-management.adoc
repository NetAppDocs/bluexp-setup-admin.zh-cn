...
[role="lead"]
BlueXP  身份和访问管理(IAM)是一种基本的可扩展访问控制机制、用于管理用户对BlueXP  资源的访问、并通过最低权限角色授予适当的权限级别。

通过IAM、您可以组织和控制对NetApp资源的访问。您可以根据组织的层次结构来组织资源。例如、您可以按地理位置、站点或业务单位来组织资源。然后、您可以将IAM角色分配给层次结构特定部分的成员、从而阻止访问层次结构其他部分中的资源。
...
用户必须具有组织管理员角色或文件夹或项目管理员角色才能管理文件夹。具有组织管理员角色的用户可以在BlueXP  组织的任何位置创建和管理资源。具有文件夹和项目管理员角色的用户只能对已分配给他们的文件夹或项目执行此操作。

链接：reference-iam-preused-roles.html[了解BlueXP  角色。]

使用BlueXP  IAM时、具有这些角色的用户将管理以下组件：
...
一个组织是BlueXP  IAM系统的顶级组织、通常代表您的公司。您的组织由连接器、存储和订阅等资源组成、您可以将这些资源组织到文件夹和项目中。然后、您可以根据组织成员(用户)在管理存储环境方面的责任为其分配对文件夹或项目的访问权限。
...
根据您的组织规模和存储资源数量、文件夹是保持NetApp资源井然有序的有用工具。如果您希望与不同用户共享不同文件夹的管理、它们还允许您更好地管理访问。您可以为用户分配特定文件夹的文件夹或项目管理员角色、然后他们可以管理对该文件夹及其项目的访问、并在该文件夹中创建新项目。

创建文件夹时、您可以为用户分配对该文件夹的访问权限。当您为用户分配文件夹访问角色时、该用户可以对相应文件夹中的所有项目执行与该角色关联的职责。相反、您可以选择按项目授予用户访问权限。
...
您可以创建多个项目、以便更精细地访问NetApp资源。例如、小型组织可能会为每个业务部门或地理位置制定一个项目。大型组织可能希望将其存储资源组织为每个业务单位的多个项目、以进一步控制访问。您可以按项目或文件夹为用户分配对资源的访问权限。
...
BlueXP  支持多个访问角色、您可以将这些角色分配给组织的成员。
